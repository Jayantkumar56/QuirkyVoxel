...
namespace Mct {
    ...
    bool ChunkMeshGenerator::ShouldDrawFace(const Block current, 
    ...
    void ChunkMeshGenerator::AddFaceIfVisible(size_t x, size_t y, size_t z,      // Current block coords
	...
	void ChunkMeshGenerator::GenerateChunkMeshes(const ChunkNeighbor& neighbor,
		                                         std::array<SubchunkCpuMeshes*, WorldConst::SubchunkCount> outMeshes)
	{
        ...
        for (size_t yIdx = 0; yIdx < WorldConst::SubchunkCount; ++yIdx) {
            ...
            GenerateSubchunkMesh(subchunkNeighbor, outMeshes[yIdx]);
        }
	}

	void ChunkMeshGenerator::GenerateSubchunkMesh(const SubChunkNeighbor& neighbor,
												  SubchunkCpuMeshes* outMesh)
	{
                                                           ...
                                                           const Block currentBlock)
        {
            ...
            SubchunkMesh& targetMesh = (currentBlock.IsWater()) ?
                outMesh->WaterMesh :
                outMesh->SolidMesh;
